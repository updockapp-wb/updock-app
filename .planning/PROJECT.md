...
## Current Milestone: 1.2.0 Refactor UI/UX & Performance
...
**Phase 5 (Recette globale & nettoyage final) — Complete (2026-07-31).** CODE-01/CODE-02/PERF-03/QA-01 validés — dernière phase du milestone 1.2.0. Les 7 contexts exposent leur hook depuis un module `use{Name}.ts` dédié, `npm run lint` intégralement vert (dette lint D-01 clôturée) ; code mort et dépendances nettoyés (`test-fcm.mjs` supprimé, `geojson` déclaré, `autoprefixer` retiré, disposition knip documentée) ; gestion d'erreurs harmonisée sur le pattern rethrow/Toast/Modal (`SpotsContext`, `AuthContext`, `AuthModal`) et `cacheSpotImages` parallélisé ; bundle JS eager réduit de 504 kB à ≈217 kB gzip (−57%, bien au-delà de la cible −15%) via `React.lazy` sur Map/AdminDashboard/PremiumModal + Error Boundary dédiée. La plateforme Android a été ajoutée au repo (jusqu'ici iOS-only, `npx cap add android`) pour permettre la recette D-11. Recette finale **approuvée par relecture qualifiée** de la checklist fusionnée (pas une exécution scénario-par-scénario sur les 2 OS) — voir `05-QA-CHECKLIST.md` §4 et `05-VERIFICATION.md` pour le détail transparent de ce qui a été vérifié vs. relu. Un incident Firebase (`GoogleService-Info.plist` embarqué par erreur dans un stash orchestrateur) a été détecté et corrigé pendant la recette. Deux limitations pré-existantes exposées par l'ajout d'Android (tag `platform: 'ios'` codé en dur sur les push tokens, deep-link `app-settings:` iOS-only) sont documentées dans `05-REVIEW.md` pour une phase future. **Milestone 1.2.0 code-complete** — `/gsd:complete-milestone` peut être lancé après une recette device exhaustive optionnelle.
...
| Milestone 1.2.0 = refactor sans nouvelle feature user | Dette technique + incohérence visuelle bloquent la maintenabilité ; consolider avant d'ajouter des features | — Pending |
| Numérotation des phases remise à 1 pour 1.2.0 | Phases v1.1.3 (01→09) archivées ; nouveau cycle repart proprement à Phase 1 | Archivé dans .planning/archive/v1.1.3-community-features/ |
...
---
*Last updated: 2026-07-31 — Phase 5 (Recette globale & nettoyage final) complete — milestone 1.2.0 code-complete*
