...
| Difficulté | `<select>` natif (AddSpotForm) | **conservé natif — hors périmètre de cette phase** | Décision planner (04-03) : le `<select>` natif est conservé ; UI-03 ne nomme que Input/Button/Modal (pas Select). La recommandation A5 (harmonisation en pills) est un changement UI optionnel repoussé, non requis par cette phase. |
